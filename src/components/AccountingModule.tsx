  ...
  const [payables, setPayables] = useState([
    {
      id: 1,
      type: 'claims',
      description: 'Motor Claim Settlement',
      amount: 2500000,
      dueDate: '2024-12-30',
      status: 'Outstanding',
      vendor: 'ABC Transport Ltd',
      treatyReference: 'TAN/MV/TTY/2024/0001'
    },
    {
      id: 2,
      type: 'commissions',
      description: 'Broker Commission - Q4 2024',
      amount: 1250000,
      dueDate: '2024-12-31',
      status: 'Pending',
      vendor: 'AON Tanzania',
      treatyReference: '12345'
    }
  ]);
...
Total Payables: USD ${(totalPayables / 1000000).toFixed(1)}M
    `;
...
Liabilities:
  Claims Payables: USD ${payables.filter(p => p.type === 'claims').reduce((sum, p) => sum + p.amount, 0).toLocaleString()}
  Commission Payables: USD ${payables.filter(p => p.type === 'commissions').reduce((sum, p) => sum + p.amount, 0).toLocaleString()}
        `;
...
Expenses:
  Claims Paid: USD ${claims.reduce((sum, c) => sum + c.claimAmount, 0).toLocaleString()}
  Commissions: USD ${payables.filter(p => p.type === 'commissions').reduce((sum, p) => sum + p.amount, 0).toLocaleString()}
        `;
            ...
            <TabsContent value="claims">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Outstanding claim payments</CardDescription>
                </CardHeader>
                <CardContent>
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {payables.filter(p => p.type === 'claims').map((payable) => (
                        <TableRow key={payable.id}>
                          ...
                          <TableCell>{payable.treatyReference}</TableCell>
                          <TableCell>
                            <Badge variant={payable.status === 'Outstanding' ? 'destructive' : 'default'}>
                              {payable.status}
                            </Badge>
                          </TableCell>
                          <TableCell>
        ...
        <TabsContent value="dashboard" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  USD {(payables.reduce((sum, p) => sum + p.amount, 0) / 1000000).toFixed(1)}M
                </div>
